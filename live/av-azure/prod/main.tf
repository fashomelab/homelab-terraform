...
locals {
  ...
  vm_name = "${local.company_abbr}-${local.environment}"                 # e.g., azlab-prod
  db_name = "${local.company_abbr}-${local.environment}-${local.region}" # e.g., azlab-prod-uks
}
...
data "vault_kv_secret_v2" "azure_credentials" {
  mount = "kv"
  name  = "apexvirtual-azure-prod/azure-credentials"
}

data "vault_kv_secret_v2" "azure_config" {
  mount = "kv"
  name  = "apexvirtual-azure-prod/azure-config"
}

data "vault_kv_secret_v2" "ssh_keys" {
  mount = "kv"
  name  = "apexvirtual-azure-prod/ssh-keys"
}
...
module "web_vm" {
  ...
  subnet_id            = module.networking.subnet_ids_by_name["av-prod-frontend"]
...
module "app_vm" {
  ...
  image_sku            = var.image_sku
  image_version        = var.image_version
  subnet_id            = module.networking.subnet_ids_by_name["av-prod-backend"]
  admin_username       = data.vault_kv_secret_v2.azure_credentials.data["admin_username"]
  admin_ssh_key_public = data.vault_kv_secret_v2.ssh_keys.data["user_public_key"]
...
module "database" {
  ...
  tags                          = var.common_tags
  subnet_id                     = module.networking.subnet_ids_by_name["av-prod-database"]
  private_dns_zone_name         = var.private_dns_zone_name
...
module "storage" {
  ...
  virtual_network_subnet_ids = [
    module.networking.subnet_ids_by_name["av-prod-frontend"],
    module.networking.subnet_ids_by_name["av-prod-backend"]
  ]
  tags = var.common_tags
}